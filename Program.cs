...
builder.Services.AddDbContext<DatabaseCtx>();
...
app.MapPut("/products/", async (IMediator mediator, [FromBody] UpdateProductInboundDto dto) => await mediator.Send(new UpdateProductCommand { Name = dto.Name, Stock = dto.Stock, Type = dto.Type, WarehouseName = dto.WarehouseName }));
